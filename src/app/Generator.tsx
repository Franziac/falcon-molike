...
export default function Generator() {
    ...
    <div className="bg-white p-6 rounded-lg shadow-lg w-1/2">
        ...
        <form onSubmit={handleSubmit} style={{ margin: '20px' }}>
            ...
            <input type="text" id="textField" className="border border-gray-300 p-2 rounded float-left w-5/6" placeholder="Enter a movie you like" value={inputValue} onChange={handleChange}></input>

            </div>
            <button type="submit" className="bg-blue-500 text-white p-2 rounded hover:bg-blue-700- float-right w-1/6">Add</button>
            <br className='m-4'></br>
            <ul>
            {items.map((item, index) => (
                <li key={index}>
                <div className='bg-slate-100 p-2 mb-1 rounded hover:bg-slate-200 w-full h-12'>
                    <p className='text-sky-500 float-left w-5/6'>{item}</p>
