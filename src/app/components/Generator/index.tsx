...
export default function Generator() {
    ...
    const requestRecommendations = async (inputs: string[], includeMovies: boolean, includeSeries: boolean) => {
        ...
        var systemPrompt = 'You are an excellent movie and tv series recommendation engine. You give personalized recommendations based on the movies and tv series the user inputs. Recommend 6 movies or tv series. Make sure you do not recommend movies or tv series the user has entered as input! Elaborate first what the movies/series have in common and what 6 movies/series would be good recommendations and why. VERY IMPORTANT: After you elaborations use the three asterisks (***) to split the elaborations with the actual recommendations wich should be in this exact format with all the properties mentioned:***{"recommendations": [{"title": "title (year)", "director": "director (on tv series leave empty)", "description": "short description of the movie or tv series", "reasoning": "short explanation of why you recommended this movie/tv series (be carefull to not include false info)", "isMovie": true (if movie) / false (if tv series)}, ...]}';
        if(includeMovies && !includeSeries) systemPrompt = 'You are an excellent movie recommendation engine. You give personalized movie recommendations based on the movies and tv series the user inputs. Recommend 6 movies. Make sure you do not recommend movies the user has entered as input! Elaborate first what the movies/series have in common and what 6 movies would be good recommendations and why. VERY IMPORTANT: After you elaborations use the three asterisks (***) to split the elaborations with the actual recommendations wich should be in this exact format with all the properties mentioned:***{"recommendations": [{"title": "movie title (year)", "director": "director", "description": "short description of the movie", "reasoning": "short explanation of why you recommended this movie (be carefull to not include false info)", "isMovie": true (always true)}, ...]}';
        else if (!includeMovies && includeSeries) systemPrompt = 'You are an excellent tv series recommendation engine. You give personalized series recommendations based on the movies and tv series the user inputs. Recommend 6 tv series. Make sure you do not recommend tv series the user has entered as input! Elaborate first what the movies/series have in common and what 6 series would be good recommendations and why. VERY IMPORTANT: After you elaborations use the three asterisks (***) to split the elaborations with the actual recommendations wich should be in this exact format with all the properties mentioned:***{"recommendations": [{"title": "title (year)", "director": "", "description": "short description of the tv series", "reasoning": "short explanation of why you recommended this tv series (be carefull to not include false info)", "isMovie": false (always false)}, ...]}';
        ...
        {
            console.log(error);
            if(res.status == 403) setResponse({data: null, status: 403});
            else setResponse({data: null, status: 500});
        }
        setRequesting(false);
        } catch (error) {
